...
function renderUsers() {
    document.getElementById('usersTableBody').innerHTML = state.users.map(function(u) {
        return '<tr><td style="padding:12px;font-weight:700">'+u.username+'</td><td style="padding:12px">'+u.email+'</td><td style="padding:12px">'+u.fullName+'</td><td style="padding:12px;text-align:center">'+roleBadge(u.role)+'</td><td style="padding:12px;text-align:center">'+(u.isActive?'<span class="nhãn nhãn-xanh-lá">Kích hoạt</span>':'<span class="nhãn nhãn-đỏ">Vô hiệu</span>')+'</td><td style="padding:12px;text-align:right"><button onclick="toggleUserStatus('+u.id+','+u.isActive+')" style="font-size:0.75rem;font-weight:700;background:none;border:none;cursor:pointer;'+(u.isActive?'color:#ef4444':'color:#059669')+'">'+(u.isActive?'Vô hiệu hóa':'Kích hoạt')+'</button></td></tr>';
    }).join('');
}
function roleBadge(role) {
    if (role === 'Admin') return '<span class="nhãn nhãn-đỏ">Admin</span>';
    if (role === 'Staff') return '<span class="nhãn nhãn-xanh-dương">Staff</span>';
    return '<span class="nhãn nhãn-xám">Customer</span>';
}
...
function renderRegRequests() {
    document.getElementById('regRequestCards').innerHTML = state.regRequests.map(function(r) {
        return '<div class="thẻ-yêu-cầu"><div class="đầu-yêu-cầu"><div><p class="tên-người-dùng">'+r.username+' <span style="color:var(--text-muted);font-weight:400">- '+r.fullName+'</span></p><p class="email-người-dùng">'+r.email+'</p><div class="chi-tiết-căn-tin"><span style="font-weight:700">Căn tin:</span> '+r.canteenName+' - '+r.canteenAddress+'<br> SĐT: '+r.canteenPhoneNumber+' | Email: '+r.canteenEmail+'</div></div><div style="text-align:right"><span class="nhãn '+(r.status==='Pending'?'nhãn-cam':r.status==='Approved'?'nhãn-xanh-lá':'nhãn-đỏ')+'">'+(r.status==='Pending'?'Chờ duyệt':r.status==='Approved'?'Đã duyệt':'Từ chối')+'</span>'+
        (r.status==='Pending' ? '<div class="nhóm-nút-duyệt" style="justify-content:flex-end"><button onclick="approveRequest('+r.id+')" class="nút-duyệt-nhỏ đồng-ý">Duyệt</button><button onclick="rejectRequest('+r.id+')" class="nút-duyệt-nhỏ từ-chối">Từ chối</button></div>' : '')+
        '</div></div></div>';
    }).join('') || '<p style="text-align:center;color:var(--text-muted);font-size:0.75rem;padding:32px 0">Không có yêu cầu</p>';
}
...
function renderRevenue() {
    document.getElementById('revenueTableBody').innerHTML = state.revenue.map(function(r) {
        return '<tr><td style="padding:12px;font-weight:700;color:var(--text-dark)">'+r.canteenName+'</td><td style="padding:12px;text-align:right;font-weight:600">'+r.totalOrders+'</td><td style="padding:12px;text-align:right;font-weight:700;color:var(--blue-main)">'+formatMoney(r.totalRevenue)+'</td></tr>';
    }).join('') || '<tr><td colspan="3" style="padding:16px;text-align:center;color:var(--text-muted)">Không có dữ liệu</td></tr>';
}
