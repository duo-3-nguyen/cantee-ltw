...
var state = {
    ...
    users: [], regRequests: [], regReqFilter: 'Pending',
    revenue: [],
...
var allViews = ['viewDashboard','viewOrders','viewMenu','viewCanteen','viewUsers','viewRegRequests','viewRevenue'];

async function bootstrap() {
    try {
        state.user = await api.auth.me();
        if (state.user.role === 'Customer') { window.location.href = 'customer.html'; return; }
        if (state.user.role === 'Staff') { window.location.href = 'staff.html'; return; }
        document.getElementById('headerUser').innerText = state.user.fullName || state.user.username;
        if (state.user.role === 'Admin') {
            document.querySelectorAll('.chỉ-admin').forEach(function(el) { el.classList.remove('ẩn'); });
            state.canteens = await api.canteens.list();
        } else {
            document.querySelectorAll('.chỉ-admin').forEach(function(el) { el.classList.add('ẩn'); });
            document.getElementById('adminNavDivider').classList.add('ẩn');
...
function setActiveNav(view) {
    var navIds = ['navDashboard','navOrders','navMenu','navCanteen','navUsers','navRegRequests','navRevenue'];
...
async function navigate(view) {
    ...
    var noCanteenViews = ['users', 'regrequests', 'revenue'];
    var canteenSel = document.getElementById('canteenSelect');
    if (noCanteenViews.indexOf(view) !== -1) canteenSel.classList.add('ẩn');
    else canteenSel.classList.remove('ẩn');

    var titles = { dashboard:'Tổng quan', orders:'Đơn hàng', menu:'Thực đơn', canteen:'Căn tin', users:'Người dùng', regrequests:'Yêu cầu đăng ký', revenue:'Doanh thu' };
    ...
    else if (view === 'users') { showView('viewUsers'); await loadUsers(); }
    else if (view === 'regrequests') { showView('viewRegRequests'); await loadRegRequests(); }
    else if (view === 'revenue') { showView('viewRevenue'); await loadRevenue(); }
...
function roleBadgeEl(role) {
    var span = document.createElement('span');
    span.className = 'nhãn ';
    if (role === 'Admin') { span.className += 'nhãn-đỏ'; span.textContent = 'Admin'; }
    else if (role === 'Staff') { span.className += 'nhãn-xanh-dương'; span.textContent = 'Staff'; }
    else { span.className += 'nhãn-xám'; span.textContent = 'Customer'; }
    return span;
}
...
async function toggleExpanded(productId) {
    if (state.expandedProductId === productId) {
        document.getElementById('productMods'+productId).classList.add('ẩn');
...
async function saveProduct(e) {
    ...
    try {
        if (id) {
            ...
            var fd = new FormData();
            fd.append('name', document.getElementById('prodName').value.trim()); fd.append('description', document.getElementById('prodDesc').value.trim()); fd.append('basePriceAmount', parseFloat(document.getElementById('prodPrice').value)||0); fd.append('categoryId', parseInt(document.getElementById('prodCategory').value)); fd.append('status', document.getElementById('prodStatus').value);
...
async function loadUsers() { try { state.users = await api.users.list(); renderUsers(); } catch(e) { showToast('Lỗi: '+(e.message||''),'error'); } }
function renderUsers() {
    var tbody = document.getElementById('usersTableBody').querySelector('tbody');
    tbody.innerHTML = '';
    var tmpl = document.getElementById('template-user-row');
    state.users.forEach(function(u) {
        var clone = tmpl.content.cloneNode(true);
        clone.querySelector('.cột-username').textContent = u.username;
        clone.querySelector('.cột-email').textContent = u.email;
        clone.querySelector('.cột-họ-tên').textContent = u.fullName;
        clone.querySelector('.cột-vai-trò-người-dùng').appendChild(roleBadgeEl(u.role));
        var statusCell = clone.querySelector('.cột-trạng-thái-người-dùng');
        if (u.isActive) { statusCell.innerHTML = '<span class="nhãn nhãn-xanh-lá">Kích hoạt</span>'; }
        else { statusCell.innerHTML = '<span class="nhãn nhãn-đỏ">Vô hiệu</span>'; }
        var toggleBtn = clone.querySelector('.nút-đổi-trạng-thái-người-dùng');
        if (u.isActive) { toggleBtn.style.color = '#ef4444'; toggleBtn.textContent = 'Vô hiệu hóa'; }
        else { toggleBtn.style.color = '#059669'; toggleBtn.textContent = 'Kích hoạt'; }
        toggleBtn.addEventListener('click', function() { toggleUserStatus(u.id, u.isActive); });
        tbody.appendChild(clone);
    });
}
async function toggleUserStatus(id, currentActive) { try { await api.users.updateStatus(id, { isActive: !currentActive }); await loadUsers(); showToast('Đã cập nhật','success'); } catch(e) { showToast('Lỗi: '+(e.message||''),'error'); } }

async function loadRegRequests() { try { state.regRequests = await api.registrationRequests.list(state.regReqFilter); renderRegRequests(); } catch(e) { showToast('Lỗi: '+(e.message||''),'error'); } }
function setRegFilter(f) {
    state.regReqFilter = f;
    document.getElementById('regFilterPending').className = 'nút-tab' + (f==='Pending' ? ' đang-chọn' : '');
    document.getElementById('regFilterApproved').className = 'nút-tab' + (f==='Approved' ? ' đang-chọn' : '');
    document.getElementById('regFilterRejected').className = 'nút-tab' + (f==='Rejected' ? ' đang-chọn' : '');
    loadRegRequests();
}
function renderRegRequests() {
    var container = document.getElementById('regRequestCards');
    container.innerHTML = '';
    if (state.regRequests.length === 0) {
        container.innerHTML = '<p style="text-align:center;color:var(--text-muted);font-size:0.75rem;padding:32px 0">Không có yêu cầu</p>';
        return;
    }
    var tmpl = document.getElementById('template-reg-request-card');
    state.regRequests.forEach(function(r) {
        var clone = tmpl.content.cloneNode(true);
        clone.querySelector('.tên-người-dùng').innerHTML = r.username + ' <span style="color:var(--text-muted);font-weight:400">- ' + r.fullName + '</span>';
        clone.querySelector('.email-người-dùng').textContent = r.email;
        clone.querySelector('.chi-tiết-căn-tin').innerHTML = '<span style="font-weight:700">Căn tin:</span> ' + r.canteenName + ' - ' + r.canteenAddress + '<br> SĐT: ' + r.canteenPhoneNumber + ' | Email: ' + r.canteenEmail;

        var statusSpan = clone.querySelector('.trạng-thái-yêu-cầu');
        if (r.status === 'Pending') { statusSpan.className = 'nhãn nhãn-cam'; statusSpan.textContent = 'Chờ duyệt'; }
        else if (r.status === 'Approved') { statusSpan.className = 'nhãn nhãn-xanh-lá'; statusSpan.textContent = 'Đã duyệt'; }
        else { statusSpan.className = 'nhãn nhãn-đỏ'; statusSpan.textContent = 'Từ chối'; }

        var btnGroup = clone.querySelector('.nhóm-nút-duyệt');
        if (r.status === 'Pending') {
            clone.querySelector('.đồng-ý').addEventListener('click', function() { approveRequest(r.id); });
            clone.querySelector('.từ-chối').addEventListener('click', function() { rejectRequest(r.id); });
        } else {
...
async function approveRequest(id) { try { await api.registrationRequests.approve(id); await loadRegRequests(); showToast('Đã duyệt','success'); } catch(e) { showToast('Lỗi: '+(e.message||''),'error'); } }
async function rejectRequest(id) { try { await api.registrationRequests.reject(id); await loadRegRequests(); showToast('Đã từ chối','success'); } catch(e) { showToast('Lỗi: '+(e.message||''),'error'); } }

async function loadRevenue() {
    var params = {};
    var from = document.getElementById('revDateFrom').value; var to = document.getElementById('revDateTo').value;
    if (from) params.from = from; if (to) params.to = to;
    try { state.revenue = await api.dashboard.revenue(params); renderRevenue(); } catch(e) { showToast('Lỗi: '+(e.message||''),'error'); }
}
function renderRevenue() {
    var tbody = document.getElementById('revenueTableBody').querySelector('tbody');
    tbody.innerHTML = '';
    if (state.revenue.length === 0) {
        tbody.innerHTML = '<tr><td colspan="3" style="padding:16px;text-align:center;color:var(--text-muted)">Không có dữ liệu</td></tr>';
        return;
    }
    var tmpl = document.getElementById('template-revenue-row');
    state.revenue.forEach(function(r) {
        var clone = tmpl.content.cloneNode(true);
        clone.querySelector('.cột-tên-căn-tin-dt').textContent = r.canteenName;
        clone.querySelector('.cột-tổng-đơn-dt').textContent = r.totalOrders;
        clone.querySelector('.cột-doanh-thu-dt').textContent = formatMoney(r.totalRevenue);
        tbody.appendChild(clone);
    });
}
